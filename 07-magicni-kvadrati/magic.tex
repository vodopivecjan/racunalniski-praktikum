...
\usepackage{hyperref} % ker imajo nekateri citati povezave
\usepackage{url}
\usepackage{graphicx}
\usepackage{xcolor}
\usepackage{calc}
...
% Definiramo okolje 'izrek'
{\theoremstyle{plain}
\newtheorem{izrek}{Izrek}
}
% definiramo okolje 'definicja'
...
{
    % Check if the optional argument is empty
    \ifx\relax#1\relax % If the optional argument is empty
        % Define the default value based on the mandatory argument
        \def\temp{Magični kvadrat reda #2}
    \else
        % Otherwise, use the provided optional argument
        \def\temp{#1}
    \fi
    % Now we begin the table
    \begin{table}[h]
         \caption{\temp}
         \label{#3}
         \centering
...
\date{\vspace{-5ex}}
% warning, da ni set author... da da mir"
\author{\vspace{-5ex}}
...
\paragraph{Magični kvadrat v litografiji Melancholia I} (glej sliko \ref{fig:durer}
